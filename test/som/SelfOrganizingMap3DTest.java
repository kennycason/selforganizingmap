...
public class SelfOrganizingMap3DTest {
	...
	public void Test() {
		...
		config.samples = new AbstractWeightVector[]{
				new WeightVectorND(0.0, 0.0, 0.0),
				new WeightVectorND(0.5, 0.5, 0.5),
				new WeightVectorND(0.9, 0.9, 0.9),
				};
		...
		WeightVectorND sample = new WeightVectorND(0.8, 0.8, 0.8);
		...
		sample = new WeightVectorND(0.9, 0.9, 0.9);
		...
		sample = new WeightVectorND(0.3, 0.5, 0.5);
		processed = som.processInput(sample);
		for(int i = 0; i < config.samples.length; i++) {
			System.out.println(config.samples[i] + " => " + processed[i]);
		}
