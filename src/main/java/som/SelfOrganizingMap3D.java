...
public class SelfOrganizingMap3D extends AbstractSelfOrganizingMap {
		...
		for (int z = 0; z < config.dimZ; z++) {
			for (int y = 0; y < config.dimY; y++) {
				for (int x = 0; x < config.dimX; x++) {
					weights[x][y][z] = new WeightVector(Vector.buildDefaultValues(config.weightVectorDimension, 0.0));
				}
			}
		}
	}
	...
	public void scaleNeighbors(AbstractMapLocation locIn, AbstractWeightVector actualIn, double t2) {
		...
		WeightVector outer = new WeightVector(Vector.buildDefaultValues(config.weightVectorDimension, R2));
		WeightVector center = new WeightVector(Vector.buildDefaultValues(config.weightVectorDimension, 0)); // default values of 0.0
